...
void statusTextCallback(const std_msgs::String::ConstPtr& msg) {
    ...
    QMetaObject::invokeMethod(qApp, [formattedMessage]() {
        if(label_main) {
            QString currentText = label_main->text();
            QString newText = currentText.isEmpty() ? formattedMessage : (currentText + "<br>" + formattedMessage);
            label_main->setText(newText);

            // Use global scroll_area
